...
test("session ingestion stores validated events and artifacts", async (context) => {
  ...
  const secondRun = await app.inject({ method: "POST", url: `/api/v1/sessions/${sessionId}/script/run`, payload: {} });
  assert.equal(secondRun.statusCode, 200);
  const detailWithExecutions = await app.inject({ method: "GET", url: `/api/v1/sessions/${sessionId}` });
  assert.equal(detailWithExecutions.json().executions.length, 2);
  assert.equal(detailWithExecutions.json().lastExecution.id, secondRun.json().id);
  const firstExecutionScreenshot = await app.inject({ method: "GET", url: run.json().screenshotUrl });
  assert.equal(firstExecutionScreenshot.statusCode, 200);
  const emptyAnalysis = await app.inject({ method: "POST", url: `/api/v1/sessions/${sessionId}/bug-analysis`, payload: { description: " " } });
  assert.equal(emptyAnalysis.statusCode, 400);
  const analysis = await app.inject({ method: "POST", url: `/api/v1/sessions/${sessionId}/bug-analysis`, payload: { description: "点击购买后没有进入支付页面" } });
  assert.equal(analysis.statusCode, 200);
  assert.equal(analysis.json().provider, "mock");
  assert.equal(analysis.json().actualBehavior, "点击购买后没有进入支付页面");
  const detailWithReport = await app.inject({ method: "GET", url: `/api/v1/sessions/${sessionId}` });
  assert.equal(detailWithReport.json().bugReport.id, analysis.json().id);
  ...
  assert.equal(deleted.json().executions, undefined);
